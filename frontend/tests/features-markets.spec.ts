...
test('markets feature: browse market list and open detail', async ({ page, request }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Tous les marchés' })).toBeVisible();
...
test('markets feature: create market from UI', async ({ page, request }) => {
  ...
  await page.getByPlaceholder('Fournissez des détails sur ce marché...').fill(marketDescription);
